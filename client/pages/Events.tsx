...
import { getEvents, getLocations } from "../../shared/api";
    ...
    const loadEvents = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          // Map API data to our Event interface format
          const apiEvents = result.data.map((event: any, index: number) => ({
            id: event.id?.toString() || index.toString(),
            title: event.title || event.name || "Untitled Event",
            description: event.description || "No description available",
            location: event.location || "Campus",
            coordinates: event.coordinates || { lat: 13.3548, lng: 74.7872 },
