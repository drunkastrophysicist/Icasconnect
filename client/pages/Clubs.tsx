    ...
    const loadClubs = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          // Map API data to our clubs interface format
          const apiClubs = result.data.map((club: any) => ({
            id: club.id?.toString() || club.name?.toLowerCase().replace(/\s+/g, '-'),
            name: club.name || "Unnamed Club",
            description: club.description || "No description available",
            category: club.category || "General",
            members: club.members || 0,
            status: club.status || "Active",
            icon: Code, // Default icon, could be mapped based on category
            gradient: "from-blue-500 to-purple-600", // Default gradient
            statusColor: club.status === "Recruiting" ? "bg-green-500/20 text-green-400" : "bg-blue-500/20 text-blue-400"
          }));
          setClubsData(apiClubs.length > 0 ? apiClubs : clubs);
        } else {
          setClubsData(clubs); // Use mock data as fallback
        }
      } catch (error) {
        console.error("Failed to load clubs:", error);
        setClubsData(clubs); // Use mock data on error
      } finally {
        setLoading(false);
      }
    };
