...
import { getMe, updateUser } from "../../shared/api";
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          // Update avatar if available from API
          if ((result.data as any).avatar) {
            setAvatar((result.data as any).avatar);
          }
          // Update name if available from API
          if ((result.data as any).name) {
            setTempName((result.data as any).name);
          }
        }
      } catch (error) {
