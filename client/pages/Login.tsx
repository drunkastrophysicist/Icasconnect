...
export default function Login() {
  ...
  const handleStudentLogin = async (e: React.FormEvent) => {
    ...
    try {
      // In offline mode, just use any valid email format with password123
      if (password === "password123") {
        await login("school"); // Use AuthProvider login
        toast({
          title: "Welcome back!",
        ...
        setError("Use password: password123 for offline mode");
        toast({
          title: "Login failed",
          description: "Use password: password123 for offline mode",
          variant: "destructive",
        });
      }
    } catch (err) {
  ...
  const handleTeacherLogin = async (e: React.FormEvent) => {
    ...
    try {
      // In offline mode, just use any valid email format with password123
      if (password === "password123") {
        await login("faculty"); // Use AuthProvider login
        toast({
          title: "Welcome back!",
  ...
  const handleSchoolLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        await login("school"); // Update auth context
        toast({
          title: "Welcome back!",
          description: "Successfully signed in with your school account",
        });
        const from = location.state?.from?.pathname || "/";
